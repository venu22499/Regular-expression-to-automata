//convert to nfa, dfa, simulation given a regex
...
void postfix_to_nfa(string postfix)
{
    for(unsigned int i=0; i<postfix.size(); i++)
    {
        ...
        case 'c': character(postfix[i]-'a'); break;
...
void display_nfa()
{
    ...
    for(unsigned int i=0; i<nfa.size(); i++)
    {
        ...
        cout<<"\t|\t";
        // for(unsigned int j=0; j<nfa[i].a[2].size(); j++)cout<<nfa[i].a[2][j]<<' ';
        // cout<<"\t|\t";
...
set<int> state_change(int c,set<int>&si){
    ...
    // {
    //     for (std::set<int>::iterator it=si.begin(); it!=si.end(); ++it){
    //         for(unsigned int j=0;j<nfa[*it].a[2].size();j++){
    //             temp.insert(nfa[*it].a[2][j]);
    //         }
    //     }   
    // }
    return temp;
